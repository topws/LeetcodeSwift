...
class ViewController: UIViewController {
	...
	// MARK: - simple 数据源
    let simpleArr: [String] = ["给定一个整数数组 nums 和一个目标值 target，请你在该数组中找出和为目标值的那 两个 整数，并返回他们的数组下标。\n 你可以假设每种输入只会对应一个答案。但是，你不能重复利用这个数组中同样的元素。\n示例:\n给定 nums = [2, 7, 11, 15], target = 9\n因为 nums[0] + nums[1] = 2 + 7 = 9\n所以返回 [0, 1]",
    ...
    var simple:[String : [String]] {
        return ["simple" : simpleArr]
    }
    // MARK: - middle数据源
    let middleArr = ["请你来实现一个 atoi 函数，使其能将字符串转换成整数。",
    ...
    var middle: [String:[String]] {
        return ["middle" : middleArr]
    }
    
    // MARK: - high数据源
    let highArr = ["合并 k 个排序链表，返回合并后的排序链表。请分析和描述算法的复杂度。",
